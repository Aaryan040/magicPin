...
def main():
    ...
    if LLM_PROVIDER != "ollama" and not LLM_API_KEY:
        print_warn("LLM_API_KEY is not set! Running in offline fallback mode (heuristic scoring only).")
        llm_ok = False
    ...
    if llm_ok:
        ...
        try:
            ...
            print_warn(f"Failed to create LLM provider: {e}. Falling back to offline mode.")
            llm_ok = False
    ...
    if llm_ok and llm:
        ...
        try:
            ...
            if test_response:
                ...
                print_warn("LLM returned empty response. Falling back to offline mode.")
                llm_ok = False
        except Exception as e:
            print_warn(f"LLM connection failed: {e}. Falling back to offline mode.")
            llm_ok = False

    if not llm_ok or not llm:
        # Create a mock/dummy provider that always raises an error to trigger the fallback scorer
        class DummyProvider(LLMProvider):
            def name(self) -> str:
                return "Offline Heuristic Scorer"
            def complete(self, prompt: str, system: str = None) -> str:
